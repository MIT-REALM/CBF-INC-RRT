...
def cartesian_to_spherical(xyz):
    ...
    if isinstance(xyz, np.ndarray):
        ...
        ptsnew[:, 0] = np.sqrt(xy + xyz[:, 2] ** 2 + 1e-6)
    elif isinstance(xyz, torch.Tensor):
        ptsnew = torch.zeros((xyz.shape[0], 4)).type_as(xyz)
        ptsnew[:, 0] = torch.sqrt(xy + xyz[:, 2] ** 2 + 1e-6)
    ptsnew[:, 1] = xyz[:, 0] / ptsnew[:, 0]
    ptsnew[:, 2] = xyz[:, 1] / ptsnew[:, 0]
    ptsnew[:, 3] = xyz[:, 2] / ptsnew[:, 0]
    return ptsnew

def spherical_to_cartesian(rxyz):
    ptsnew = torch.zeros((rxyz.shape[0], 3)).type_as(rxyz)
    ptsnew[:, 0] = rxyz[:, 1] * rxyz[:, 0]
    ptsnew[:, 1] = rxyz[:, 2] * rxyz[:, 0]
    ptsnew[:, 2] = rxyz[:, 3] * rxyz[:, 0]
    return ptsnew

def do_spherical_do_cartesian(rxyz):
    xyz = spherical_to_cartesian(rxyz)
    jacobian = torch.zeros((rxyz.shape[0], 4, 3))
    # partial_r/partial_xi
    for x_ind in range(3):
        jacobian[:, 0, x_ind] = xyz[:, x_ind] / rxyz[:, 0]
    # partial_ri/partial_xi
    for x_ind in range(3):
        for y_ind in range(3):
            jacobian[:, x_ind + 1, y_ind] = int(x_ind==y_ind) / rxyz[:, 0] - xyz[:, x_ind] * xyz[:, y_ind]/ rxyz[:, 0] ** 3
    return jacobian
