...
if TYPE_CHECKING:
    from neural_cbf.controllers import Controller  # noqa
...
class ExperimentSuite(object):
    ...
    def find_exp_instance(self, exp_name: str):
        """
        Find corresponding experiment instance from experiment suite.
        Return the first instance found.
        """
        for experiment in self.experiments:
            if type(experiment) == exp_name:
                return experiment
        raise KeyError(f"Exp_name: {exp_name} not found in experiment suite [{[type(i) for i in self.experiments]}]")
