...
from .search_tree_dd import SearchTree_dd, insert_new_state

def RRT_plan_dd(**kwargs):
...
def NEXT_plan(env, dynamics_model, init_state, goal_state, RRT_PARAM, controller=None,
    ...
    search_tree = SearchTree_dd(root=init_state)
...
def global_explore(search_tree, dynamics_model, sample_state=None, steer_type='line', controller=None,
    ...
    finish_sample = False
    while not finish_sample:
        # Steer sample to nearby location
        dists = np.linalg.norm(non_terminal_states - sample_state, axis=-1)
        nearest_idx = np.argmin(dists)
        if search_tree.vert_radius[nearest_idx] > np.linalg.norm(sample_state - non_terminal_states[nearest_idx]):
            finish_sample = True
        else:
            sample_state = dynamics_model.sample_state_space(1).numpy().squeeze()
    t1 = time.time()
    ...
    for _ in range(RRT_PARAM//30):
        ...
        ttt1 = time.time()
        if not no_collision:
            search_tree.vert_radius[parent_idx] = 0.1 * dynamics_model.n_dims
